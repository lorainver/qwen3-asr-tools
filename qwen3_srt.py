...
"""
Qwen3-ASR 字幕生成脚本
用法: python qwen3_srt.py <音视频文件> [--model 0.6B|1.7B] [--lang zh|en|auto] [--device cpu|cuda] [--cache_dir <目录>]
"""
import argparse
import sys
import time
...
def main():
    ...
    parser.add_argument("--device", default="cpu", choices=["cpu", "cuda"],
                        help="推理设备 (默认: cpu)")
    parser.add_argument("--cache_dir", default=r"D:\qwen3-asr\models",
                        help="模型缓存目录")
    ...
    if args.output:
        ...
        output_path = audio_path.with_name(f"{audio_path.stem}_qwen3-{args.model}.srt")
    ...
    seg_list = list(results) if hasattr(results, '__iter__') else results

    print(f"耗时: {elapsed:.1f}s ({elapsed/60:.1f}min) | 段数: {len(seg_list)}")
