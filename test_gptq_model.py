"""
Test GPTQ model loading with auto-gptq
"""
import torch
...
if hasattr(config, 'quantization_config') and config.quantization_config:
    print(f"   Quantization method: {config.quantization_config.quant_method if hasattr(config.quantization_config, 'quant_method') else 'GPTQ'}")
print()
