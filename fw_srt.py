...
def main():
    ...
    p.add_argument("--compute", default="float16",
                   help="计算精度，默认 float16。可选 float32/float16/int8/int8_float16")
    p.add_argument("--beam", type=int, default=5, help="Beam size，越大精度越高但越慢，默认 5")
    ...
    try:
        ...
        if "CUBLAS_STATUS_NOT_SUPPORTED" in str(e) and args.compute == "int8_float16":
            print(f"警告: 当前 GPU 不支持 int8_float16，尝试回退到 float16...")
            model = faster_whisper.WhisperModel(
                args.model, device="cuda", compute_type="float16"
            )
        elif "CUBLAS_STATUS_NOT_SUPPORTED" in str(e) and args.compute == "int8":
            print(f"警告: 当前 GPU 不支持 int8，尝试回退到 float16...")
            model = faster_whisper.WhisperModel(
                args.model, device="cuda", compute_type="float16"
            )
        else:
    ...
    segments_iter, info = model.transcribe(
        ...
        task="transcribe",
        beam_size=args.beam,       # 使用用户指定的 beam_size
    )
    print(f"音频时长: {info.duration:.0f}s  |  语言: {info.language}  |  beam: {args.beam}")
