...
def _format_system_message_text(text):
    if not text:
        return ""
    if '<sysmsg' not in text:
        return text
    
    # 1. 尝试匹配 <content>...</content> (撤回消息等)
    content_match = re.search(r'<content>(.*?)</content>', text)
    if content_match:
    ...
    # 2. 尝试匹配 <plain><![CDATA[...]]></plain> (系统模板消息如邀请、扫码加入、群名片修改等)
    plain_match = re.search(r'<plain>(?:\s*<\!\[CDATA\[)?(.*?)(?:\]\]>\s*)?</plain>', text, re.DOTALL)
    if plain_match:
...
def _format_message_text(local_id, local_type, content, is_group, chat_username, chat_display_name, names, display_name_fn, db_dir=None, create_time_ts=0, resolve_media=False):
    ...
    if base_type == 3:
        ...
        text = _format_system_message_text(text)
    elif base_type != 1:
...
def _build_history_line(row, ctx, names, id_to_username, display_name_fn, resolve_media=False, db_dir=None):
    ...
    if sender_label:
        return create_time, f'[{time_str}] {sender_label} <{sender_username}>: {text}'
    return create_time, f'[{time_str}] <{sender_username}>: {text}'
